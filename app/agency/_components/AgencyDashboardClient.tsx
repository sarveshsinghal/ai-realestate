...
type Props = {
  ...
  needsAttention: Array<{
    ...
    score: number;
    grade: string;
    flags: { missingBasics: boolean; missingPrice: boolean; weakScore: boolean };
  }>;
  recentLeads: Array<{
    id: string;
    name: string;
...
function timeAgo(iso: string) {
  ...
  return new Intl.DateTimeFormat("en-GB", { day: "2-digit", month: "short", year: "numeric" }).format(from);
}

function statusBadgeVariant(s: LeadStatus) {
  // Keep within shadcn "variant" spirit by using className
  switch (s) {
    case "NEW":
...
function sparkColorIndex(status: LeadStatus): number {
  // Maps to CSS vars you already have: --chart-1..5 (shadcn chart tokens)
  // Use deterministic indices; Recharts uses "Cell" fill, but we keep it minimal and theme-driven via CSS vars.
  switch (status) {
    case "NEW":
      return 1;
    case "CONTACTED":
      ...
      <div className="mx-auto max-w-6xl space-y-6 px-6 py-8">
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-4">
          <KpiCard
            ...
            chip={props.kpis.totalListings > 0 ? `${Math.round((props.kpis.publishedCount / props.kpis.totalListings) * 100)}% live` : "—"}
          />
          <KpiCard
            title="Leads"
          ...
          <KpiCard
            ...
            chip={props.kpis.avgScore >= 75 ? "Strong" : props.kpis.avgScore >= 60 ? "OK" : "Needs work"}
          />
          <KpiCard
            title="Strong deals"
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
          ...
          <Card className="md:col-span-2">
            ...
            <CardContent className="h-[220px]">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={props.leadsTrend} margin={{ top: 10, right: 16, left: 0, bottom: 0 }}>
                  <CartesianGrid strokeDasharray="3 3" />
                  <XAxis dataKey="date" tick={{ fontSize: 11 }} tickFormatter={(v) => String(v).slice(5)} />
                  <YAxis tick={{ fontSize: 11 }} allowDecimals={false} />
                  <ReTooltip />
                  <Line type="monotone" dataKey="count" stroke={chartFill(2)} strokeWidth={2} dot={false} />
                </LineChart>
              </ResponsiveContainer>
            </CardContent>
          </Card>
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
          <Card className="md:col-span-2">
            <CardHeader className="flex flex-row items-center justify-between space-y-0">
              ...
              <Link href="/agency/listings" className="text-sm underline underline-offset-4">
                  ...
                  >
                    <div className="min-w-0">
                      <div className="flex flex-wrap items-center gap-2">
                        <div className="truncate text-sm font-semibold">{l.title ?? "(Untitled)"}</div>
                        <span className={cx("rounded-full border px-2 py-0.5 text-xs font-semibold", scoreBadgeTone(l.grade))}>
                          {l.grade}
                        </span>
          ...
          <Card className="md:col-span-1">
            ...
            <CardContent className="h-[260px]">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={props.scoreDistribution} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                  <CartesianGrid strokeDasharray="3 3" />
                  ...
                  <Bar dataKey="count">
                    {props.scoreDistribution.map((x, idx) => (
                      <Cell key={x.bucket} fill={chartFill(((idx % 5) + 1) as 1 | 2 | 3 | 4 | 5)} />
                    ))}
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0">
              ...
              <Link href="/agency/leads" className="text-sm underline underline-offset-4">
                View all <ArrowUpRight className="ml-1 inline h-4 w-4" />
              </Link>
            </CardHeader>

            <CardContent>
              <Tabs value={leadTab} onValueChange={(v) => setLeadTab(v as any)} className="w-full">
                        ...
                        <div className="flex items-start justify-between gap-3">
                          <div className="min-w-0">
                            ...
                            <div className="mt-1 text-sm text-muted-foreground">
                              {l.listing?.title ? (
                                <>
                                  <span className="font-medium text-foreground">{l.listing.title}</span>
                                  {l.listing.commune ? <span className="text-muted-foreground"> • {l.listing.commune}</span> : null}
                                </>
                              ) : l.listingId ? (
                                <span className="text-muted-foreground">Listing linked</span>
                              ) : (
                                <span className="text-muted-foreground">Unassigned listing</span>
                              )}
                            </div>
                            ...
                            <div className="mt-3 flex flex-wrap items-center gap-2">
                              ...
                              {l.phone ? (
                                <Tooltip>
                                  ...
                                  <TooltipContent>Copy phone to clipboard</TooltipContent>
                                </Tooltip>
                              ) : null}
                              ...
                              {l.listingId ? (
                                <Button size="sm" variant="outline" asChild>
                                  <Link href={`/agency/listings/${l.listingId}/edit`}>Open listing</Link>
                                </Button>
                              ) : null}
                            </div>
                          </div>
  ...
  return (
    <Card className="relative overflow-hidden">
      ...
      <CardHeader className="flex flex-row items-start justify-between space-y-0">
        ...
        <div className="rounded-xl border bg-card p-2 text-muted-foreground">{props.icon}</div>
      </CardHeader>
      <CardContent>
